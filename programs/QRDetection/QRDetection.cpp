...
#include "QRDetection.hpp"


/************************************************************************/
bool QRDetection::configure(yarp::os::ResourceFinder &rf) {
    ...
    printf("QRDetection using cropSelector: %d.\n",cropSelector);
    ...
    printf("QRDetection using kinectDevice: %s, kinectLocal: %s, kinectRemote: %s.\n",
        strKinectDevice.c_str(), strKinectLocal.c_str(), strKinectRemote.c_str());
    printf("QRDetection using watchdog: %f.\n",watchdog);
    ...
    if (!rf.check("help")) {
        ...
        printf("[QRDetection] success: kinectDevice available.\n");
        if (! dd.view(kinect) ) fprintf(stderr,"[QRDetection] warning: kinectDevice bad view.\n");
        else printf("[QRDetection] success: kinectDevice ok view.\n");
...
/*****************************************************************/
double QRDetection::getPeriod() {
    return watchdog;  // [s]
}

/************************************************************************/

bool QRDetection::updateModule() {
    printf("QRDetection alive...\n");
    return true;
}

/************************************************************************/

bool QRDetection::interruptModule() {
    printf("QRDetection closing...\n");
